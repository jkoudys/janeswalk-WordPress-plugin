...
/**
 * Methods to manage the walk table
 */
document.addEventListener('DOMContentLoaded', function() {
  // Convenience methods
  var forEach = Function.prototype.call.bind(Array.prototype.forEach);

  // Load the walk list table
  ...
  if (walkList) {
    ...
    forEach(walkList.querySelectorAll('thead th'), function(option) {
      option.addEventListener('click', function(ev) {
        ...
        if (this.classList.contains('sort')) {
          // If this is the currently sorted-by column, just reverse their order
          ...
          // Add the sorted list of walks to the page
          walks.forEach(function(walk) {
            walkBody.appendChild(walk);
          });

          // Clear the sort arrows, set to the new one
          forEach(sortOptions, function(option) {
            option.classList.remove('sort', 'reverse');
          });
